    ...
    const fetchImages = async () => {
      try {
        ...
        if (currentCategory === 'All Photos') {
          response = await axios.get('http://savor-restaurant-1.onrender.com/api/Gallery');
        } else if(currentCategory === 'VIP Guests'){
          response = await axios.get('http://savor-restaurant-1.onrender.com/api/Gallery?category={VIP}')
        }else {
          response = await axios.get(`http://savor-restaurant-1.onrender.com/api/Gallery?category=${currentCategory}`);
        }
