    ...
    const fetchImages = async () => {
      try {
        ...
        if (currentCategory === 'All Photos') {
          response = await axios.get(`${process.env.BACKEND_URL}/api/Gallery`);
        } else if(currentCategory === 'VIP Guests'){
          response = await axios.get(`${process.env.BACKEND_URL}/api/Gallery?category={VIP}`)
        }else {
          response = await axios.get(`${process.env.BACKEND_URL}/api/Gallery?category=${currentCategory}`);
        }
